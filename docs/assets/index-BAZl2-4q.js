...
class GameHelper {
  constructor() {
    __publicField(this, "eliminateTheLine", 2 ** gameParam.column - 1);
  }
  getRandomLetter() {
  ...
  drawBg(ctx, colors, brickWidth = gameParam.brickWidth, brickHeight = gameParam.brickHeight) {
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
...
class EventEmitter {
  ...
  on(event, listener) {
    ...
    (_a = this.events[event]) == null ? void 0 : _a.push(listener);
  }
  emit(event, ...args) {
...
class Ui {
  ...
  addEventEmitter() {
    ...
    this.eventEmitter.on("updateNextBrick", (brick) => {
      this.dom.nextBrickCanvas.getContext("2d").clearRect(
        0,
        0,
